            ...
            ratingHtml = `
                <div class="rating">
                    ...
                    <select id="rating-${game.id}">
                        ${Array.from({
                length: 10
            }, (_, i)=>`<option value="${i + 1}" ${savedRating == i + 1 ? "selected" : ""}>${i + 1}</option>`).join("")}
                    </select>/10
                </div>
            `;
        }
...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    function updateSelectedFilters() {
        ...
        if (selectedDecade) selectedFiltersText += `Decade - ${selectedDecade.textContent}`;
        if (selectedPlatform) {
    ...
    if (decade1990) decade1990.addEventListener("click", (event)=>{
        event.preventDefault();
    ...
    if (decade2000) decade2000.addEventListener("click", (event)=>{
        event.preventDefault();
    ...
    if (decade2010) decade2010.addEventListener("click", (event)=>{
        event.preventDefault();
    ...
    if (decade2020) decade2020.addEventListener("click", (event)=>{
        event.preventDefault();
    ...
    if (allDecades) allDecades.addEventListener("click", (event)=>{
        event.preventDefault();
        baseUrl = `https://api.rawg.io/api/games?key=${apiKey}&platforms=4,18,1,7`;
        callAPI(1);
    });
    ...
    if (platformPC) platformPC.addEventListener("click", (event)=>{
        event.preventDefault();
    ...
    if (platformPlaystation) platformPlaystation.addEventListener("click", (event)=>{
        event.preventDefault();
        baseUrl = `https://api.rawg.io/api/games?key=${apiKey}&platforms=18`;
        callAPI(1);
    });
    const platformXbox = document.getElementById("platform-xbox");
    if (platformXbox) platformXbox.addEventListener("click", (event)=>{
        event.preventDefault();
    ...
    if (platformCommodore) platformCommodore.addEventListener("click", (event)=>{
        event.preventDefault();
        baseUrl = `https://api.rawg.io/api/games?key=${apiKey}&platforms=10`;
        callAPI(1);
    });
    const platformAll = document.getElementById("platform-all");
    if (platformAll) platformAll.addEventListener("click", (event)=>{
        event.preventDefault();
